                ...
                {isReady ? (
                  ...
                  <div style={{ textAlign:'center', padding:'20px 0' }}>
                    ...
                    <div style={{ fontSize:13, color:'var(--gray-500)' }}>
                      {status?.error ? `Error: ${status.error}` : 'Initializing WhatsApp...'}
                    </div>
                    <div style={{ fontSize:12, color:'var(--gray-400)', marginTop:6 }}>
                      {status?.error ? 'Check Render backend logs for details' : 'This takes 15-30 seconds on first run'}
                    </div>
                    {status?.error && (
                      <div style={{ marginTop:10, padding:'8px 12px', background:'var(--red-50)', border:'1px solid var(--red-100)', borderRadius:8, fontSize:12, color:'var(--red-600)' }}>
                        {status.error}
                      </div>
                    )}
                    <div style={{ display:'flex', gap:8, marginTop:12, justifyContent:'center' }}>
                      <button className="btn" onClick={() => { refetchQr(); refetchStatus(); }}>
                        ↺ Check Again
                      </button>
                      <button className="btn btn-primary" onClick={() => restartMutation.mutate()}>
                        ⟳ Restart WhatsApp
                      </button>
                    </div>
                  </div>
