...
router.get('/status', async (req, res) => {
  try {
    ...
    res.json({ connected: true, merchantCode: c.merchantCode, pickupAddressCode: c.pickupAddressCode });
  ...
  try {
    ...
    for (const url of URLS) {
      try {
        ...
        if (r.data && r.data.statusCode === '200') { baseUrl = url; break; }
      } catch(e) {}
    }
    ...
    await supabase.from('settings').upsert({ key: 'postex_connection', value: JSON.stringify({ token, merchantCode, pickupAddressCode, baseUrl, connectedAt: new Date().toISOString(), citiesCount: r.data.dist ? r.data.dist.length : 0 }) }, { onConflict: 'key' });
    res.json({ success: true, merchantCode: merchantCode, citiesCount: r.data.dist ? r.data.dist.length : 0 });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/test', async (req, res) => {
  try {
    ...
    res.json({ success: true, cities: r.data.dist ? r.data.dist.length : 0 });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
